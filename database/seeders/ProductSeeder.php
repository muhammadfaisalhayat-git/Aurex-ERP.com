...
class ProductSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($products as $productData) {
            $product = Product::updateOrCreate(['code' => $productData['code']], $productData);
        }

        $bundle = Product::where('code', 'BUNDLE-PC-01')->first();
        $laptop = Product::where('code', 'LAPTOP-001')->first();
        $monitor = Product::where('code', 'MONITOR-001')->first();
        $keyboard = Product::where('code', 'KEYBOARD-001')->first();
        $mouse = Product::where('code', 'MOUSE-001')->first();

        if ($bundle && $laptop && $monitor && $keyboard && $mouse) {
            ProductBom::updateOrCreate(['product_id' => $bundle->id, 'component_id' => $laptop->id], ['quantity' => 1, 'waste_percentage' => 0]);
            ProductBom::updateOrCreate(['product_id' => $bundle->id, 'component_id' => $monitor->id], ['quantity' => 2, 'waste_percentage' => 0]);
            ProductBom::updateOrCreate(['product_id' => $bundle->id, 'component_id' => $keyboard->id], ['quantity' => 1, 'waste_percentage' => 0]);
            ProductBom::updateOrCreate(['product_id' => $bundle->id, 'component_id' => $mouse->id], ['quantity' => 1, 'waste_percentage' => 0]);
        }

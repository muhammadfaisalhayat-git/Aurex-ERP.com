...
@section('content')
                                ...
                                <div class="card-body">
                                    <div class="row g-3 align-items-end">
                                        <div class="col-md-3">
                                            ...
                                            <input type="date" id="m_from_date" class="form-control" value="{{ $fromDate }}">
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label small fw-bold">{{ __('messages.to_date') }}</label>
                                            <input type="date" id="m_to_date" class="form-control" value="{{ $toDate }}">
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label small fw-bold">{{ __('messages.warehouse') }}</label>
                                            <select id="m_warehouse_id" class="form-select select2">
                                        ...
                                        <div class="col-md-2">
                                            <button type="button" onclick="applyMovementFilters()" class="btn btn-secondary w-100">
                                        ...
                                        @forelse($movements as $movement)
                                            ...
                                            <tr>
                                                <td class="small fw-bold">
                                                    @if($routeName)
                                                        <a href="{{ route($routeName, $movement->reference_id) }}" class="text-primary text-decoration-none">
                                                            <i class="fas fa-external-link-alt small me-1"></i> {{ $movement->reference_number }}
                                                        </a>
                                                    @else
                                                    ...
                                                    <span class="badge bg-soft-info text-info">
                                                        @php
                                                            $docTypeLabels = [
                                                                ...
                                                                'stocksupply'      => 'Stock Supply',
                                                                'stockreceiving'   => 'Stock Receiving',
                                                                'stocktransfer'    => 'Stock Transfer',
                                                                'stockissueorder'  => 'Stock Issue',
                                                            ...
                                                            $refKey = strtolower($movement->reference_type);
                                                            $docLabel = $docTypeLabels[$refKey] ?? ucwords(str_replace('_', ' ', $refKey));
                                                        @endphp
                                                        {{ $docLabel }}
                                                    </span>
                                                </td>
                                                ...
                                                <td class="small text-end fw-bold {{ $movement->balance_quantity < 0 ? 'text-danger' : 'text-primary' }}">
    ...
    <script>
        function applyMovementFilters() {
            ...
            url.hash = 'movement'; // Keep the tab active
            window.location.href = url.toString();
        }

        // Auto-switch to movement tab if hash is present
        document.addEventListener('DOMContentLoaded', function() {

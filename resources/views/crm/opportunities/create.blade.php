...
@section('title', __('crm.create_opportunity'))

@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3">{{ __('crm.create_opportunity') }}</h1>
            ...
            <div class="card-body p-4">
                <form action="{{ route('crm.opportunities.store') }}" method="POST">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-8">
                            <label class="form-label fw-bold">{{ __('crm.opportunity_title') }} <span class="text-danger">*</span></label>
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title') }}" required placeholder="{{ __('crm.opportunity_placeholder') }}">
                            @error('title') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-4">
                            <label class="form-label fw-bold">{{ __('crm.pipeline_stage') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="lead_id" class="form-select select2">
                                ...
                                @foreach($leads as $lead)
                                    <option value="{{ $lead->id }}" {{ (old('lead_id') == $lead->id || $selectedLeadId == $lead->id) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            <label class="form-label fw-bold">{{ __('crm.expected_revenue') }} <span class="text-danger">*</span></label>
                            <div class="input-group">
                                <span class="input-group-text">{{ config('app.currency', 'SAR') }}</span>
                                <input type="number" step="0.01" name="expected_revenue" class="form-control @error('expected_revenue') is-invalid @enderror" value="{{ old('expected_revenue', 0) }}" required>
                            </div>
                            @error('expected_revenue') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-4">
                            <label class="form-label fw-bold">{{ __('crm.probability') }} (%) <span class="text-danger">*</span></label>
                            <input type="number" name="probability" class="form-control @error('probability') is-invalid @enderror" value="{{ old('probability', 10) }}" required min="0" max="100">
                        ...
                        <div class="col-12 text-end mt-5">
                            <button type="submit" class="btn btn-primary btn-lg px-5 shadow-sm">
                                <i class="fas fa-save me-2"></i> {{ __('crm.save_opportunity') }}
                            </button>
                        </div>
                    </div>
                </form>

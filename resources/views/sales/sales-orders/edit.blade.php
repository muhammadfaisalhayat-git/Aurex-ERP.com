            ...
            document.addEventListener('turbo:load', function () {
                ...
                @if(old('items'))
                    @foreach(old('items') as $item)
                        ...
                        addItem({ product_id: "{{ $item['product_id'] }}", product: @json($prod), quantity: "{{ $item['quantity'] }}", unit_price: "{{ $item['unit_price'] }}", tax_rate: "{{ $prod->tax_rate ?? 0 }}", discount_percentage: "{{ $item['discount_percentage'] ?? 0 }}" });
                    @endforeach
                @else

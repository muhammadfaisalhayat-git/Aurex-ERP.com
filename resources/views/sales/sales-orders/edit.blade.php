...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3">{{ __('messages.edit_sales_order') }}</h1>
            <a href="{{ route('sales.sales-orders.show', $salesOrder) }}" class="btn btn-secondary">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            <label for="document_number"
                                class="form-label fw-bold">{{ __('messages.document_number') }}</label>
                            <input type="text" class="form-control bg-light" id="document_number"
                                value="{{ $salesOrder->document_number }}" readonly>
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control bg-white @error('order_number') is-invalid @enderror"
                                ...
                                value="{{ old('order_number', $salesOrder->order_number) }}" required readonly>
                            @error('order_number')
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date"
                                ...
                                value="{{ old('expected_delivery_date', $salesOrder->expected_delivery_date ? $salesOrder->expected_delivery_date->format('Y-m-d') : '') }}">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <div class="position-relative">
                                <input type="text" id="customer_search"
                                    ...
                                    value="{{ old('customer_search', $salesOrder->customer->name_en ?? $salesOrder->customer->name_ar) }}">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select bg-white @error('branch_id') is-invalid @enderror" id="branch_id"
                                ...
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ (old('branch_id', $salesOrder->branch_id)) == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name_en }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select bg-white @error('salesman_id') is-invalid @enderror" id="salesman_id"
                                ...
                                @foreach(\App\Models\User::active()->get() as $salesman)
                                    <option value="{{ $salesman->id }}" {{ (old('salesman_id', $salesOrder->salesman_id)) == $salesman->id ? 'selected' : '' }}>
                                        {{ $salesman->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <table class="table table-hover mb-0" id="itemsTable">
                            <thead>
                                <tr class="bg-light">
                                    ...
                                    <th style="width: 14%">{{ __('messages.quantity') }} /
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea class="form-control bg-white" id="delivery_address" name="delivery_address"
                                rows="3">{{ old('delivery_address', $salesOrder->delivery_address) }}</textarea>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea class="form-control bg-white" id="terms_conditions" name="terms_conditions"
                                rows="3">{{ old('terms_conditions', $salesOrder->terms_conditions) }}</textarea>
                        </div>
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea class="form-control bg-white" id="notes" name="notes"
                                rows="2">{{ old('notes', $salesOrder->notes) }}</textarea>
                        </div>
                    </div>
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn btn-primary px-5 py-2 fw-bold">
                            <i class="fas fa-save me-2"></i> {{ __('messages.update_order') ?? 'Update Sales Order' }}
                        </button>
                    </div>
                </div>
            </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('turbo:load', function () {
                ...
                // Customer Search Logic (Same as Create)
                ...
                function addItem(data = null) {
                    ...
                    if (data && data.product && data.product.units) {
                        data.product.units.forEach(u => {
                            ...
                            if (results.length > 0) {
                                ...
                                resultsDiv.querySelectorAll('.search-result-item').forEach(item => {
                                    item.addEventListener('click', function () {
                                        ...
                                        if (stock <= 0) {
                                            Swal.fire({ icon: 'warning', title: 'Out of Stock', text: 'Insufficient stock in selected warehouse.' });
                                            resultsDiv.style.display = 'none';
                                            searchInput.value = '';
                                            return;
                                        }
                ...
                function calculateTotals() {
                    let subtotal = 0; let taxTotal = 0; let grandTotal = 0;
                ...
                // Initial Items or Old Data
                @if(old('items'))
                    @foreach(old('items') as $item)
                        @php $prod = \App\Models\Product::find($item['product_id']); @endphp
                        addItem({ product_id: "{{ $item['product_id'] }}", measurement_unit_id: "{{ $item['measurement_unit_id'] ?? '' }}", product: @json($prod), quantity: "{{ $item['quantity'] }}", unit_price: "{{ $item['unit_price'] }}", tax_rate: "{{ $prod->tax_rate ?? 0 }}", discount_percentage: "{{ $item['discount_percentage'] ?? 0 }}" });
                    @endforeach
                @else

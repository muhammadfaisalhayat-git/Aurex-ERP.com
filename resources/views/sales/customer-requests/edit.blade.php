...
@section('title', __('messages.edit_customer_request') ?? 'Edit Customer Request')

@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3">{{ __('messages.edit_customer_request') ?? 'Edit Customer Request' }}</h1>
            <a href="{{ route('sales.customer-requests.index') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-left"></i> {{ __('messages.back') }}
            </a>
        </div>

        <form action="{{ route('sales.customer-requests.update', $customerRequest) }}" method="POST"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control bg-white @error('document_number') is-invalid @enderror"
                                ...
                                value="{{ old('document_number', $customerRequest->document_number) }}" required readonly>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" class="form-control bg-white @error('request_date') is-invalid @enderror"
                                ...
                                value="{{ old('request_date', $customerRequest->request_date->format('Y-m-d')) }}" required>
                            @error('request_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" class="form-control bg-white @error('needed_date') is-invalid @enderror"
                                ...
                                value="{{ old('needed_date', $customerRequest->needed_date ? $customerRequest->needed_date->format('Y-m-d') : '') }}">
                            @error('needed_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <div class="position-relative">
                                <input type="text" id="customer_search"
                                    ...
                                    value="{{ $customerRequest->customer->name_en ?? '' }}">
                                <input type="hidden" name="customer_id" id="customer_id"
                                    value="{{ old('customer_id', $customerRequest->customer_id) }}">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select bg-white @error('branch_id') is-invalid @enderror" id="branch_id"
                                ...
                                @foreach($branches as $branch)
                                    <option value="{{ $branch->id }}" {{ old('branch_id', $customerRequest->branch_id) == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name_en }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select bg-white @error('status') is-invalid @enderror" id="status"
                                ...
                                <option value="pending" {{ old('status', $customerRequest->status) == 'pending' ? 'selected' : '' }}>{{ __('messages.pending') ?? 'Pending' }}</option>
                                <option value="draft" {{ old('status', $customerRequest->status) == 'draft' ? 'selected' : '' }}>{{ __('messages.draft') }}</option>
                                <option value="cancelled" {{ old('status', $customerRequest->status) == 'cancelled' ? 'selected' : '' }}>{{ __('messages.cancelled') ?? 'Cancelled' }}</option>
                                <option value="converted" {{ old('status', $customerRequest->status) == 'converted' ? 'selected' : '' }}>{{ __('messages.converted') ?? 'Converted' }}</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            ...
                            <textarea class="form-control bg-white" id="notes" name="notes"
                                rows="2">{{ old('notes', $customerRequest->notes) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table table-hover mb-0" id="itemsTable">
                            ...
                            <tfoot class="bg-light">
                                <tr>
                                    ...
                                    <td class="fw-bold" id="grand_subtotal">
                                ...
                                <tr>
                                    <td colspan="5" class="text-end fw-bold">
                                        {{ __('messages.tax_amount') ?? 'Tax Amount' }}:
                                    </td>
                                    <td class="fw-bold" id="grand_tax">{{ number_format($customerRequest->tax_amount, 2) }}
                                    </td>
                                    <td colspan="2"></td>
                                </tr>
                                <tr class="table-primary">
                                    ...
                                    <td class="fw-bold" id="grand_total">
        ...
        <script>
            document.addEventListener('turbo:load', function () {
                ...
                function performProductSearch(query, resultsDiv, idInput, searchInput) {
                    ...
                    fetch(`{{ route('inventory.products.ajax-search') }}?q=${encodeURIComponent(query)}&branch_id=${branchId}`)
                                                                ...
                                                                <div class="search-result-item p-2 border-bottom" 
                                                                    ...
                                                                    <div class="d-flex justify-content-between align-items-start">
                                                                        <div class="result-content">
                                                                            ...
                                                                            <small class="${stockColor} fw-bold mt-1 d-block"><i class="fas fa-boxes" style="font-size:0.65rem;"></i> ${stockLabel}: ${parseFloat(p.available_quantity || 0).toFixed(2)}</small>
                                                                        </div>

    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            ...
            @if($customerRegistration->status === 'pending')
                ...
                <a href="{{ route('customer-registrations.edit', $customerRegistration) }}" class="btn btn-primary">
            ...
            @if($customerRegistration->status === 'approved' && !$customerRegistration->converted_customer_id)
                ...
                <form action="{{ route('customer-registrations.convert', $customerRegistration) }}" method="POST" class="d-inline">
                    @csrf
                    <button type="submit" class="btn btn-success" onclick="return confirm('{{ __('customer_registration.confirm_convert') }}')">
    ...
    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <table class="table table-borderless table-sm">
                                <tr>
                                    ...
                                    <td>{{ $customerRegistration->credit_limit ? number_format($customerRegistration->credit_limit, 2) : '-' }}</td>
                                </tr>
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($customerRegistration->status === 'pending')
                                            @can('customer_registration.edit')
                                            <form action="{{ route('customer-registrations.documents.delete', $document) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="col-md-4">
                ...
                <div class="card-body">
                    <table class="table table-borderless table-sm">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('customers.show', $customerRegistration->converted_customer_id) }}">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @can('customer_registration.approve')
                    <form action="{{ route('customer-registrations.approve', $customerRegistration) }}" method="POST" class="mb-3">
                        @csrf
                        <div class="mb-3">
                            <label for="approval_notes" class="form-label">{{ __('customer_registration.approval_notes') }}</label>
                            <textarea class="form-control" id="approval_notes" name="approval_notes" rows="2"></textarea>
                        </div>
                        <button type="submit" class="btn btn-success w-100" onclick="return confirm('{{ __('customer_registration.confirm_approve') }}')">
                            <i class="fas fa-check"></i> {{ __('customer_registration.approve') }}
                        </button>
                    </form>

                    <form action="{{ route('customer-registrations.reject', $customerRegistration) }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="rejection_reason" class="form-label">{{ __('customer_registration.rejection_reason') }} *</label>
                        ...
                        <button type="submit" class="btn btn-danger w-100" onclick="return confirm('{{ __('customer_registration.confirm_reject') }}')">

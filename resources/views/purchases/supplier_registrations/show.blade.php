    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            ...
            @if($supplierRegistration->status === 'pending')
                ...
                <a href="{{ route('supplier-registrations.edit', $supplierRegistration) }}" class="btn btn-primary">
            ...
            @if($supplierRegistration->status === 'approved' && !$supplierRegistration->converted_vendor_id)
                ...
                <form action="{{ route('supplier-registrations.convert', $supplierRegistration) }}" method="POST" class="d-inline">
                    @csrf
                    <button type="submit" class="btn btn-success" onclick="return confirm('{{ __('supplier_registration.confirm_convert') }}')">
    ...
    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <table class="table table-borderless table-sm">
                                ...
                                <tr>
                                    ...
                                    <td>{{ $supplierRegistration->website ? '<a href="' . $supplierRegistration->website . '" target="_blank">' . $supplierRegistration->website . '</a>' : '-' }}</td>
                                </tr>
                            </table>
                        </div>
                        <div class="col-md-6">
                        ...
                        <table class="table table-sm">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ Storage::url($document->document_path) }}" target="_blank" class="btn btn-sm btn-info">
                                            <i class="fas fa-download"></i>
                                        </a>
                                        @if($supplierRegistration->status === 'pending')
                                            @can('supplier_registration.edit')
                                            <form action="{{ route('supplier-registrations.documents.delete', $document) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="col-md-4">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @can('supplier_registration.approve')
                    <form action="{{ route('supplier-registrations.approve', $supplierRegistration) }}" method="POST" class="mb-3">
                        @csrf
                        <div class="mb-3">
                            <label for="approval_notes" class="form-label">{{ __('supplier_registration.approval_notes') }}</label>
                            <textarea class="form-control" id="approval_notes" name="approval_notes" rows="2"></textarea>
                        </div>
                        <button type="submit" class="btn btn-success w-100" onclick="return confirm('{{ __('supplier_registration.confirm_approve') }}')">
                            <i class="fas fa-check"></i> {{ __('supplier_registration.approve') }}
                        </button>
                    </form>

                    <form action="{{ route('supplier-registrations.reject', $supplierRegistration) }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="rejection_reason" class="form-label">{{ __('supplier_registration.rejection_reason') }} *</label>
                        ...
                        <button type="submit" class="btn btn-danger w-100" onclick="return confirm('{{ __('supplier_registration.confirm_reject') }}')">

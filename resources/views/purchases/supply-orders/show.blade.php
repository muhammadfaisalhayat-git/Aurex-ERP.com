            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-1">
                        <li class="breadcrumb-item"><a href="{{ route('purchases.supply-orders.index') }}"
                                class="text-decoration-none">{{ __('messages.supply_orders') }}</a></li>
            ...
            <div class="d-flex">
                    ...
                    @can('send supply orders')
                        <form action="{{ route('purchases.supply-orders.send', $supplyOrder) }}" method="POST" class="d-inline">
                            ...
                            <button type="submit" class="btn btn-success me-2 shadow-sm"
                                onclick="return confirm('{{ __('messages.confirm_send_order') }}')">
                ...
                @if($supplyOrder->canBeInvoiced())
                    @can('convert supply orders')
                        <button type="button" class="btn btn-info text-white me-2 shadow-sm" data-bs-toggle="modal"
                ...
                <a href="{{ route('purchases.supply-orders.print', $supplyOrder) }}" target="_blank"
                ...
                <div class="card shadow-sm border-0 mb-4">
                    <div class="card-body p-4">
                        <div class="row g-4">
                            ...
                            <div class="col-md-3">
                                <p class="small text-muted text-uppercase fw-bold mb-1">
                                    {{ __('messages.expected_delivery') }}</p>
                                <p class="fw-bold mb-0 text-warning">
                                    {{ $supplyOrder->expected_delivery_date ? $supplyOrder->expected_delivery_date->format('Y-m-d') : '-' }}
                                </p>
                            </div>
                            <div class="col-md-3">
                                ...
                                @endphp
                                <span
                                    class="badge border bg-{{ $supplyOrder->status === 'draft' ? 'light' : $color }} {{ $supplyOrder->status === 'draft' ? 'text-muted' : '' }} fw-semibold rounded-pill px-3">
                            ...
                            <div class="col-md-6 border-end">
                                ...
                                <p class="mb-1 fw-bold fs-5">
                                    {{ app()->getLocale() == 'ar' ? ($supplyOrder->vendor?->name_ar ?? $supplyOrder->vendor?->name_en ?? '-') : ($supplyOrder->vendor?->name_en ?? '-') }}
                                </p>
                                <p class="text-muted mb-1"><i
                                        class="fas fa-map-marker-alt me-2"></i>{{ $supplyOrder->vendor?->address ?? '-' }}
                                </p>
                                <p class="text-muted mb-0"><i
                                        class="fas fa-phone me-2"></i>{{ $supplyOrder->vendor?->phone ?? '-' }}</p>
                            </div>
                            <div class="col-md-6 ps-4">
                                <h6 class="fw-bold mb-3 text-primary">{{ __('messages.delivery_information') }}</h6>
                                <p class="mb-1 fs-6"><span class="fw-bold text-dark">{{ __('messages.branch') }}:</span>
                                    {{ $supplyOrder->branch->name ?? '-' }}</p>
                                <p class="mb-0 fs-6"><span class="fw-bold text-dark">{{ __('messages.warehouse') }}:</span>
                                    {{ $supplyOrder->warehouse->name ?? '-' }}</p>
                                <p class="mt-2 text-muted fw-semibold small"><i
                                        class="fas fa-hashtag me-2"></i>{{ __('messages.reference_number') }}:
                                    {{ $supplyOrder->order_number }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="table table-hover align-middle mb-0">
                                <thead class="bg-light text-muted small text-uppercase fw-bold">
                                    <tr>
                                        ...
                                        <th class="py-3 text-center">{{ __('messages.quantity') }} /
                                    ...
                                    @foreach($supplyOrder->items as $index => $item)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if($item->notes)
                                                    <div class="small text-info mt-1"><i
                                                            class="fas fa-info-circle me-1"></i>{{ $item->notes }}</div>
                                                @endif
                                            </td>
                                            <td class="text-center fw-semibold">
                                                {{ number_format($item->quantity, 3) }}
                                                <small class="text-muted d-block"
                                                    style="font-size: 0.75rem;">{{ $item->measurementUnit->name ?? '' }}</small>
                                            </td>
                            ...
                            <div class="card shadow-sm border-0 h-100">
                                <div class="card-body p-4">
                                    <h6 class="fw-bold mb-3"><i
                                            class="fas fa-sticky-note me-2 text-warning"></i>{{ __('messages.notes') }}</h6>
                                    <p class="mb-0 text-muted" style="white-space: pre-line;">
                                        {{ $supplyOrder->notes ?? __('messages.no_notes') }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="card shadow-sm border-0 h-100">
                                <div class="card-body p-4">
                                    <h6 class="fw-bold mb-3"><i
                                            class="fas fa-gavel me-2 text-info"></i>{{ __('messages.terms_conditions') }}</h6>
                                    <p class="mb-0 text-muted" style="white-space: pre-line;">
                                        {{ $supplyOrder->terms_conditions ?? __('messages.no_terms') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-3">
                ...
                <div class="card shadow-sm border-0 bg-primary text-white mb-4 overflow-hidden" style="min-height: 200px;">
                    <div class="card-body p-4 position-relative z-index-1">
                        <i class="fas fa-file-invoice fa-2x position-absolute opacity-25"
                ...
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-white border-bottom-0 pt-4 px-4">
                        <h6 class="fw-bold mb-0"><i
                                class="fas fa-history me-2 text-secondary"></i>{{ __('messages.status_history') }}</h6>
                    </div>
                    <div class="card-body p-4">
                        <div class="vertical-timeline ps-3 border-start">
                            @foreach($supplyOrder->statusHistory->sortByDesc('created_at') as $history)
                                <div class="timeline-item mb-4 position-relative">
                                    <div class="timeline-point rounded-circle bg-white border border-primary position-absolute"
                                        style="width: 12px; height: 12px; left: -21px; top: 4px;"></div>
                                    ...
                                    <p class="small text-muted mt-1 align-items-center d-flex fw-semibold border-top pt-1"
                ...
                <form action="{{ route('purchases.supply-orders.convert-to-invoice', $supplyOrder) }}" method="POST">
                    ...
                    <div class="modal-header bg-info text-white">
                        <h5 class="modal-title fw-bold"><i
                                class="fas fa-exchange-alt me-2"></i>{{ __('messages.convert_to_invoice') }}</h5>
                    ...
                    <div class="modal-footer bg-light border-0">
                        ...
                        <button type="submit"
                            class="btn btn-info text-white px-4 fw-bold">{{ __('messages.confirm_convert') }}</button>
                    </div>
                </form>
            </div>
        </div>

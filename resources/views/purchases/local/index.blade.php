    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            @can('local_purchase.create')
                <a href="{{ route('purchases.local-purchases.create') }}" class="btn btn-primary">
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            @forelse($purchases as $purchase)
                                <tr>
                                    <td>
                                        <a href="{{ route('purchases.local-purchases.show', $purchase) }}">
                                    ...
                                    <td>
                                        <span
                                            class="badge bg-{{ $purchase->status === 'posted' ? 'success' : ($purchase->status === 'draft' ? 'warning' : 'secondary') }}">
                                        ...
                                        <div class="btn-group">
                                            ...
                                            @if($purchase->status === 'draft')
                                                @can('local_purchase.edit')
                                                    <a href="{{ route('purchases.local-purchases.edit', $purchase) }}"
                                                        class="btn btn-sm btn-primary">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                @endcan
                                                @can('local_purchase.post')
                                                    <form action="{{ route('purchases.local-purchases.post', $purchase) }}"
                                            ...
                                            @elseif($purchase->status === 'posted')
                                                @can('local_purchase.post')
                                                    <form action="{{ route('purchases.local-purchases.unpost', $purchase) }}"
                                                        method="POST" class="d-inline">
                                                        @csrf
                                                        @method('PATCH')

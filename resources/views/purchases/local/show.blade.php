            ...
            <div>
                ...
                @if($localPurchase->status === 'draft')
                    @can('local_purchase.edit')
                        <a href="{{ route('purchases.local-purchases.edit', $localPurchase) }}" class="btn btn-primary">
                            <i class="fas fa-edit"></i> {{ __('general.edit') }}
                        </a>
                    @endcan
                    @can('local_purchase.post')
                        <form action="{{ route('purchases.local-purchases.post', $localPurchase) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-success"
                                onclick="return confirm('{{ __('local_purchase.confirm_post') }}')">
                                <i class="fas fa-check"></i> {{ __('local_purchase.post') }}
                            </button>
                        </form>
                    @endcan
                    @can('local_purchase.delete')
                        <form action="{{ route('purchases.local-purchases.destroy', $localPurchase) }}" method="POST"
                            class="d-inline">
                            @csrf
                            @method('DELETE')
                ...
                @elseif($localPurchase->status === 'posted')
                    @can('local_purchase.post')
                        <form action="{{ route('purchases.local-purchases.unpost', $localPurchase) }}" method="POST"
                            class="d-inline">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-warning"
                                onclick="return confirm('{{ __('local_purchase.confirm_unpost') }}')">
            ...
            <div class="col-md-8">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <span
                            class="badge bg-{{ $localPurchase->status === 'posted' ? 'success' : ($localPurchase->status === 'draft' ? 'warning' : 'secondary') }}">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            {{ number_format($localPurchase->items->sum(fn($i) => ($i->quantity * $i->unit_price)), 2) }}
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr class="table-primary">
                                        ...
                                        <td class="text-end fw-bold">{{ number_format($localPurchase->gross_amount, 2) }}
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>

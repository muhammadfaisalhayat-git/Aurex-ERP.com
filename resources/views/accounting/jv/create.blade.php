...
@section('content')
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3 mb-0 text-gray-800">{{ __('messages.create_journal_voucher') }}</h1>
                        ...
                        <table class="table table-bordered" id="items-table">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th width="20%">{{ __('messages.details') }}</th> <!-- Entity/Sub-Ledger -->
                            ...
                            <tbody>
                                <tr class="item-row">
                                    <td>
                                        <select name="items[0][main_account_id]"
                                            class="form-select select2 main-account-select" required data-row="0">
                                            <option value="">{{ __('messages.select_main_account') }}</option>
                                        </select>
                                    </td>
                                    <td>
                                        <select name="items[0][chart_of_account_id]"
                                            class="form-select select2 sub-account-select" required disabled>
                                    ...
                                    <td><input type="number" name="items[0][debit]" class="form-control debit-input"
                                            step="0.01" value="0.00" min="0"></td>
                                    <td><input type="number" name="items[0][credit]" class="form-control credit-input"
                                            step="0.01" value="0.00" min="0"></td>
                                ...
                                <tr class="item-row">
                                    <td>
                                        <select name="items[1][main_account_id]"
                                            class="form-select select2 main-account-select" required data-row="1">
                                            <option value="">{{ __('messages.select_main_account') }}</option>
                                        </select>
                                    </td>
                                    <td>
                                    ...
                                    <td class="entity-cell">
                                        <select name="items[1][customer_id]"
                                            class="form-select select2-ajax customer-select d-none" data-type="customer">
                                            <option value="">{{ __('messages.select_customer') }}</option>
                                        </select>
                                    ...
                                    <td><input type="number" name="items[1][debit]" class="form-control debit-input"
                                            step="0.01" value="0.00" min="0"></td>
                                    <td><input type="number" name="items[1][credit]" class="form-control credit-input"
                                            step="0.01" value="0.00" min="0"></td>
                                    <td><input type="text" name="items[1][notes]" class="form-control"></td>
                                    <td></td>
                                </tr>
                            </tbody>
                            <tfoot>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Flatten accounts for easier lookup if needed, but we rely on traversal

                function populateMainAccounts(selectElement) {
                ...
                function populateSubAccounts(mainId, subSelectElement) {
                    ...
                    // Enable select if we have options, even if they are all disabled (so user can see them)
                    // Or only if we have at least one enabled option?
                    // Select2 might handle disabled options by graying them out.
                            ...
                            processResults: function (data) {
                                return {
                                    results: data.map(function (item) {
                                        return {
                                            id: item.id,
                                            text: item.name_en || item.name_ar || item.name // Adjust based on API response
                                        };
                                    })
                                };
                            },
                ...
                // Initial population
                document.querySelectorAll('.main-account-select').forEach(select => {
                    populateMainAccounts(select);
                });
                // Init AJAX select2
                ...
                tableBody.addEventListener('change', function (e) {
                    if (e.target.classList.contains('main-account-select')) {
                        ...
                        handleSubAccountChange(subSelect); // Reset entity fields
                ...
                addRowBtn.addEventListener('click', () => {
                    ...
                    const newSelect = newRow.querySelector('.main-account-select');
                    populateMainAccounts(newSelect);

                    // Initialize select2
                    if (typeof $ !== 'undefined' && $.fn.select2) {
                        $(newRow).find('.select2').select2({ theme: 'bootstrap-5' });
                        newRow.querySelectorAll('.select2-ajax').forEach(el => initAjaxSelect2(el));
                    }

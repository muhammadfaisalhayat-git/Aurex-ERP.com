                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            @if(!$role->is_system)
                                @can('edit roles')
                                    <a href="{{ route('acp.user-mgmt.roles.edit', $role) }}" class="btn btn-primary">
                                        <i class="fas fa-edit"></i> {{ __('messages.edit_role') }}
                                    </a>
                                    <a href="{{ route('acp.user-mgmt.roles.permissions', $role) }}" class="btn btn-outline-primary">
                                        <i class="fas fa-user-shield"></i> {{ __('messages.edit_permissions') }}
                                    </a>
                                @endcan
                                ...
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        @foreach($role->users as $user)
                                            <tr>
                                                <td>
                                                    <a href="{{ route('acp.user-mgmt.users.show', $user) }}">{{ $user->name }}</a>
                                                </td>
                                                <td>{{ $user->email }}</td>

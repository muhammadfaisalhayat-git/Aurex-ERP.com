...
@section('title', __('messages.transport') . ' - ' . __('messages.edit_contract'))

@section('content')
    <div class="page-header">
        <div>
            <h1 class="page-title">{{ __('messages.edit_contract') }}: {{ $contract->contract_number }}</h1>
    ...
    <div class="row">
        <div class="col-md-9">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('transport.contracts.update', $contract) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    <option value="pending" {{ old('status', $contract->status) == 'pending' ? 'selected' : '' }}>{{ __('messages.pending') }}</option>
                                    <option value="active" {{ old('status', $contract->status) == 'active' ? 'selected' : '' }}>{{ __('messages.active') }}</option>
                                    <option value="closed" {{ old('status', $contract->status) == 'closed' ? 'selected' : '' }}>{{ __('messages.closed') }}</option>
                                    <option value="cancelled" {{ old('status', $contract->status) == 'cancelled' ? 'selected' : '' }}>{{ __('messages.cancelled') }}</option>
                                </select>
                            ...
                            <div class="col-12 text-end mt-4">
                                ...
                                <button type="submit" class="btn btn-primary px-5">
                                    <i class="fas fa-save me-2"></i>{{ __('messages.update_contract') }}
                                </button>
                            </div>
                        </div>
                    </form>

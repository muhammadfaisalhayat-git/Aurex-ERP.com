...
@section('content')
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3 mb-0 text-gray-800">New Payment Voucher</h1>
            <a href="{{ route('finance.vouchers.payment.index') }}" class="btn btn-secondary">
        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                <form action="{{ route('finance.vouchers.payment.store') }}" method="POST">
                    @csrf
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            <label class="form-label">Expense/Target GL Account <span class="text-danger">*</span></label>
                            <select name="chart_of_account_id" id="target_account_id" class="form-select select2" required>
                                <option value="">Select Account</option>
                                @foreach($coaAccounts as $coa)
                                    <option value="{{ $coa->id }}" data-sub-ledger="{{ $coa->sub_ledger_type }}">
                                        {{ $coa->code }} - {{ $coa->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4 mb-3">
                    ...
                    <div class="row" id="beneficiary_container" style="display: none;">
                        <div class="col-md-12 mb-3">
                            <div class="p-3 bg-light border rounded">
                                <label class="form-label fw-bold" id="beneficiary_label">Select Beneficiary (Mandatory for
                                    this Account) <span class="text-danger">*</span></label>
                                ...
                                <div class="form-text text-primary" id="beneficiary_help">This account requires a specific
                                    entity for subsidiary ledger tracking.</div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('turbo:load', function () {
                ...
                fetch(`/accounting/gl/coa/${accountId}/beneficiaries`)
                    ...
                    .then(data => {
                        if (data.beneficiaries && data.beneficiaries.length > 0) {
                            ...
                            beneficiaryTypeInput.value = data.beneficiaries[0].type; // Assuming same type for all entries in the set
            ...
            beneficiarySelect.addEventListener('change', function () {
                if (this.value && !payeeInput.value) {
                    payeeInput.value = this.options[this.selectedIndex].text;
                }
            });

            // Initialize on load if there's a selected account (e.g., validation error redirect)

...
@section('title', __('messages.payment_vouchers') . ' - ' . __('messages.finance_banking'))

@section('content')
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('messages.payment_vouchers') }}</h1>
        <a href="{{ route('finance.vouchers.payment.create') }}" class="btn btn-primary">
            <i class="fas fa-plus me-1"></i> {{ __('messages.new_payment_voucher') }}
        </a>
    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <a href="{{ route('finance.vouchers.payment.show', $voucher->id) }}" class="btn btn-outline-primary">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if($voucher->status == 'draft')
                                            <form action="{{ route('finance.vouchers.payment.post', $voucher->id) }}" method="POST" class="d-inline">
                                                @csrf
